...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.c_fc = nn.Linear(config.n_embd, MLP_EXPAND * config.n_embd, bias=False)
        self.c_proj = nn.Linear(MLP_EXPAND * config.n_embd, config.n_embd, bias=False)
...
class MoELayer(nn.Module):
    def __init__(self, config):
        ...
        self.num_selected = NUM_SELECTED_EXPERTS
    ...
    def forward(self, x):
        ...
        weights = F.softmax(logits, dim=-1, dtype=torch.bfloat16)
        top_weights, top_indices = torch.topk(weights, self.num_selected, dim=-1)
        top_weights = top_weights / top_weights.sum(dim=-1, keepdim=True)
        top_weights = top_weights.to(x.dtype)

        out = torch.zeros_like(flat_x)
        for sel in range(self.num_selected):
            sel_idx = top_indices[:, sel]
            sel_w = top_weights[:, sel]
            for i, expert in enumerate(self.experts):
                mask = sel_idx == i
                if mask.any():
                    out[mask] += sel_w[mask].unsqueeze(-1) * expert(flat_x[mask])
